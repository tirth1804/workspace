...
void stressGetKeys(dict *d, int times, int *perfect_run, int *approx_run) {
    ...
    for (j = 0; j < times; j++) {
        ...
        if (requested == returned && dup == 0) {
            ...
            (*approx_run)++;
            print(f("Requested, returned, duplicated: %d %d %d\n",
...
int main(void) {
    ...
    print(f("Stress testing dictGetSomeKeys\n");
    ...
    print(f("dictGetSomeKey, %d perfect runs, %d approximated runs\n",
