    ...
    unsigned i)
{
	...
	if (config_tcache) {
		malloc_cprint(f(write_cb, cbopaque,
	...
	for (j = 0, in_gap = false; j < nbins; j++) {
		...
		if (nruns == 0)
			...
			assert(milli <= 1000);
			if (milli < 10) {
				malloc_snprint(f(util, sizeof(util),
				    "0.00%zu", milli);
			} else if (milli < 100) {
				malloc_snprint(f(util, sizeof(util), "0.0%zu",
				    milli);
			} else if (milli < 1000) {
				malloc_snprint(f(util, sizeof(util), "0.%zu",
				    milli);
			} else
				malloc_snprint(f(util, sizeof(util), "1");
    ...
    unsigned i, bool bins, bool large, bool huge)
{
	...
	malloc_cprint(f(write_cb, cbopaque, "dss allocation precedence: %s\n",
	    dss);
    ...
    const char *opts)
{
	...
	if (general) {
		...
		malloc_cprint(f(write_cb, cbopaque, "Version: %s\n", cpv);
		CTL_GET("config.debug", &bv, bool);
		malloc_cprint(f(write_cb, cbopaque, "Assertions %s\n",
		    bv ? "enabled" : "disabled");
...
#undef OPT_WRITE_CHAR_P
		...
		malloc_cprint(f(write_cb, cbopaque, "CPUs: %u\n", ncpus);

		CTL_GET("arenas.narenas", &uv, unsigned);
		malloc_cprint(f(write_cb, cbopaque, "Arenas: %u\n", uv);

		malloc_cprint(f(write_cb, cbopaque, "Pointer size: %zu\n",
		    sizeof(void *));

		CTL_GET("arenas.quantum", &sv, size_t);
		malloc_cprint(f(write_cb, cbopaque, "Quantum size: %zu\n",
		    sv);

		CTL_GET("arenas.page", &sv, size_t);
		malloc_cprint(f(write_cb, cbopaque, "Page size: %zu\n", sv);
	...
	malloc_cprint(f(write_cb, cbopaque, "--- End jemalloc statistics ---\n");
}
