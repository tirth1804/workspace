...
prof_dump_open(bool propagate_err, const char *filename)
{
	...
	if (fd == -1 && !propagate_err) {
		malloc_print(f("<jemalloc>: creat(\"%s\"), 0644) failed\n",
		    filename);
		if (opt_abort)
			abort();
	}
...
JEMALLOC_FORMAT_print(F(2, 3)
static bool
prof_dump_print(f(bool propagate_err, const char *format, ...)
{
	bool ret;
	va_list ap;
	char buf[PROF_print(F_BUFSIZE];
...
prof_tctx_dump_iter(prof_tctx_tree_t *tctxs, prof_tctx_t *tctx, void *arg)
{
	...
	case prof_tctx_state_dumping:
	case prof_tctx_state_purgatory:
		if (prof_dump_print(f(propagate_err,
    ...
    prof_gctx_tree_t *gctxs)
{
	...
	for (i = 0; i < bt->len; i++) {
		if (prof_dump_print(f(propagate_err, " %#"FMTxPTR,
		    (uintptr_t)bt->vec[i])) {
			ret = true;
			goto label_return;
		}
...
prof_open_maps(const char *format, ...)
{
	...
	va_start(ap, format);
	malloc_vsnprint(f(filename, sizeof(filename), format, ap);
	va_end(ap);
    ...
    const char *filename)
{
	...
	if (cnt_all->curbytes != 0) {
		malloc_print(f("<jemalloc>: Leak summary: %"FMTu64" byte%s, %"
...
prof_dump_filename(char *filename, char v, uint64_t vseq)
{
	...
	if (vseq != VSEQ_INVALID) {
		...
		malloc_snprint(f(filename, DUMP_FILENAME_BUFSIZE,
