...
TEST_BEGIN(test_malloc_snprint(f_truncated)
{
#define	BUFLEN	15
	char buf[BUFLEN];
	int result;
...
#define TEST(expected_str_untruncated, ...) do {			\
	result = malloc_snprint(f(buf, len, __VA_ARGS__);		\
...
TEST_BEGIN(test_malloc_snprint(f)
{
#define	BUFLEN	128
	char buf[BUFLEN];
	int result;
#define	TEST(expected_str, ...) do {					\
	result = malloc_snprint(f(buf, sizeof(buf), __VA_ARGS__);	\
...
main(void)
{
	...
	return (test(
	    ...
	    test_malloc_snprint(f_truncated,
	    test_malloc_snprint(f));
}
