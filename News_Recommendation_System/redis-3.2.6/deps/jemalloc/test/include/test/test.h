...
#define	assert_cmp(t, a, b, cmp, neg_cmp, pri, ...) do {		\
	...
	if (!(a_ cmp b_)) {						\
		...
		malloc_snprint(f(prefix, sizeof(prefix),			\
		    "%s:%s:%d: Failed assertion: "			\
		    "(%s) "#cmp" (%s) --> "				\
		    "%"pri" "#neg_cmp" %"pri": ",			\
		    __func__, __FILE__, __LINE__,			\
		    #a, #b, a_, b_);					\
		malloc_snprint(f(message, sizeof(message), __VA_ARGS__);	\
...
void	test_skip(const char *format, ...) JEMALLOC_FORMAT_print(F(1, 2);
void	test_fail(const char *format, ...) JEMALLOC_FORMAT_print(F(1, 2);
